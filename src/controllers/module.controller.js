...
const getActiveModules = async (req, res) => {
    try {
        ...
        // Filter out any installed modules where module is null (deleted)
        const validModules = installedModules.filter(im => im.module !== null);

        const activeModules = validModules.map(im => ({
            id: im.module._id,
            name: im.module.name,
            slug: im.module.slug,
            icon: im.module.icon,
...
const uninstallModule = async (req, res) => {
    try {
        ...
        // Find the installed module record first
        ...
        if (dependentModules.length > 0) {
            // Check each dependent module to see if it's installed
            const installedDependents = [];
            
            for (const dep of dependentModules) {
                const isInstalled = await InstalledModule.findOne({
                    organization: organizationId,
                    module: dep._id,
                    status: { $in: ['active', 'trial'] }
                });
                ...
                if (isInstalled) {
                    installedDependents.push(dep.name);
                }
            }
        ...
        // Uninstall module - use deleteOne with the specific ID
        await InstalledModule.deleteOne({ _id: installedModule._id });
        
        // Also check if there are any orphaned records with the same moduleSlug
        await InstalledModule.deleteMany({
            organization: organizationId,
            moduleSlug: slug,
            module: null  // Delete any orphaned records
        });
