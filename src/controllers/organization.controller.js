...
const updateSettings = async (req, res) => {
    try {
        // Get organization ID from headers, request body, user, or query params
        let organizationId = req.headers['x-organization-id'] || 
                            req.body.organizationId || 
                            req.user?.organizationId ||
                            req.query.organizationId;

        // If still not found, try to get from the organization context in request
        ...
        if (!organizationId) {
            return res.status(400).json({
                success: false,
                message: 'Organization ID not provided. Please include x-organization-id header or organizationId in request body.'
            });
        }
        ...
        // Find settings with proper organization ID
        ...
        if (!settings) {
            settings = new OrganizationSettings({
                organization: organizationId,
                createdBy: req.member?.id || req.user?.id
            });
            await settings.save();
        }
        ...
        if (acceptedCurrencies !== undefined) settings.acceptedCurrencies = acceptedCurrencies;
        if (taxSystem !== undefined) settings.taxSystem = taxSystem;
        if (taxRates !== undefined) settings.taxRates = taxRates;
        if (defaultLanguage !== undefined) settings.defaultLanguage = defaultLanguage;
        
        // Merge password policy if provided
        if (passwordPolicy) {
        ...
        if (notificationChannels !== undefined) settings.notificationChannels = notificationChannels;
        ...
        // Set updated by user
        settings.updatedBy = req.member?.id || req.user?.id;
        await settings.save();
